...
export class WalletServiceUrlPage {
  ...
  ionViewWillEnter() {
    ...
    this.comment = this.replaceParametersProvider.replace(
      this.translate.instant(
        "{{appName}} depends on Bitcore Wallet Service (BWS) for blockchain information, networking and Copayer synchronization. The default configuration points to https://bws.vpubchain.club (Particl's public BWS instance)."
      ),
      { appName }
    );
  ...
  public save(): void {
    ...
    switch (this.walletServiceForm.value.bwsurl) {
      case 'prod':
      case 'production':
        bws = 'https://bws.vpubchain.club/bws/api';
        break;
      case 'sta':
      case 'staging':
